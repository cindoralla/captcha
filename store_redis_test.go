...
func TestRedisSetGet(t *testing.T) {
	s, err := NewRedisStore(&redis.Options{Addr: "localhost:6379", DB: 0, Password: "root123456"}, 1*time.Minute, DefaultMaxRedisKeys, DefaultRedisPrefixKey)
	if err != nil {
		t.Errorf(err.Error())
	}
...
func TestRedisGetClear(t *testing.T) {
	s, err := NewRedisStore(&redis.Options{Addr: "localhost:6379", DB: 0, Password: "root123456"}, Expiration, DefaultMaxRedisKeys, DefaultRedisPrefixKey)
	if err != nil {
		t.Errorf(err.Error())
	}
	id := "redis-id"
